...
export default function MenuNav() {
  ...
  return (
    <header className="px-4 h-14 fixed z-50 md:z-100 flex w-full bg-backgrounStartRgb justify-between items-center">
      ...
      <Link
        href="/"
        className={`items-center ml-10 my-10 ${
          searchToggle ? "flex" : "hidden"
        }`}>
      ...
      <div
        ...
        className={`block md:hidden cursor-pointer p-2.5 hover:bg-youtube2 rounded-full ${
          searchToggle ? "hidden" : "block"
        }`}>
        <ArrowLeft size={20} />
      </div>
      <div
        className={`md:flex items-center w-2/5 ${
          searchToggle ? "hidden" : "w-full flex"
        }`}>
        <div id="search" className="flex w-full">
          <div
            className={`flex bg-backgrounInput items-center border rounded-s-3xl h-10 w-full ${
              isActive
                ? "border-sky-700 ml-0 pr-0 pl-0"
                : "border-youtube2 pr-1 pl-4 ml-8"
            }`}>
            <div
              id="search-active"
              className={`pr-3 pl-4 ${isActive ? "flex" : "hidden"}`}>
              <Search size={20} />
            </div>
            <input
              type="text"
            ...
            />
            <div
              className={`mx-1 p-2 ${isSearchNotEmpty ? "flex" : "hidden"} ${
                isActive ? "mr-2" : ""
              } hover:rounded-full hover:bg-secondaireRgb cursor-pointer `}
              onClick={clearSearch}>
              <X size={20} />
            </div>
          </div>
        ...
        <div className="ml-4 relative group cursor-pointer hover:bg-youtube2 p-2 rounded-full bg-secondaireRgb">
          <Mic size={24} />
          <div className="opacity-0 whitespace-nowrap group-hover:opacity-100 ease-in duration-100 pointer-events-none top-14 absolute left-0 p-2 rounded-md bg-neutral-600/95">
            <h4 className="text-xs">Search with your voice</h4>
          </div>
        </div>
      </div>
      <div
